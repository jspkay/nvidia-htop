...
parser.add_argument(
    "-l", "--command-length", default=20, const=100, type=int, nargs="?"
)
parser.add_argument(
    "-L", "--username-length", default=40, const=100, type=int, nargs="?"
)
...
parser.add_argument(
    ...
    "--meter",
    action="store_true",
    help="Shows meters for GPU and VRAM utilization",
)
parser.add_argument(
    "-u",
    "--user",
    default="",
    help="Limit the list of processes to selected users (comma-separated)",
)
parser.add_argument(
    "-i",
    "--id",
    default="",
    help="Limit the command to selected GPU IDs (comma-separated)",
)
# only for testing
parser.add_argument(
    "-p",
    "--fake-ps",
    help="The list of processes to use instead of real output of `ps`",
)
...
if fake_stdin_path is not None:
    ...
    if len(args.id) > 0:
        print(
            "nvidia-htop argument -i/--id cannot be used when nvidia-smi output is being piped into it. To filter the"
            " shown GPUs, pass the -i argument to the nvidia-smi call instead.",
            file=sys.stderr,
        )
else:
    nvidiasmi_args = []
    ...
    ps_call = subprocess.run(
        ["nvidia-smi"] + nvidiasmi_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    if ps_call.returncode != 0:
        print(
            "nvidia-smi exited with error code {}:".format(ps_call.returncode),
            file=sys.stderr,
        )
...
def add_meters(_lines):
    ...
    for i, line in gpu_util_lines:
        ...
        gpu_meter = "|" * round(gpu_util * gpu_meter_space) + " " * round(
            (1 - gpu_util) * gpu_meter_space
        )
        mem_meter = "|" * round(mem_util * mem_meter_space) + " " * round(
            (1 - mem_util) * mem_meter_space
        )
...
def colorize(_lines):
    ...
    for j in range(len(_lines)):
        ...
        if m is not None:
            ...
            is_moderate = False
            is_high = (
                gpu_util >= GPU_MODERATE_RATIO or mem_util >= MEMORY_MODERATE_RATIO
            )
            if not is_high:
                is_moderate = (
                    gpu_util >= GPU_FREE_RATIO or mem_util >= MEMORY_FREE_RATIO
                )

            c = "red" if is_high else ("yellow" if is_moderate else "green")
            # Color all lines from the `start_idx`, until we find a separator.
            for k in takewhile(
                lambda k: not lines[k].startswith("+----"),
                range(start_idx, len(_lines)),
            ):
...
if no_running_process in lines[i] or lines[i].startswith("+--"):
    ...
    if lines[i].startswith("+--"):
        print(
            "| If you're running in a container, you'll only see processes running inside. |"
        )
    print(lines[-1])
    sys.exit()
...
if fake_ps is None:
    ...
    ps_format = f"pid,user:{username_length},%cpu,%mem,etime,command"
    ps_call = subprocess.run(
...
for line in processes:
    ...
    for idx in filter(lambda p: pid[p] == parts[0], range(len(pid))):
        if users is not None and not user_exists(parts[1], users):
        ...
        mem[idx] = parts[3]
        time[idx] = (
            parts[4] if "-" not in parts[4] else parts[4].split("-")[0] + " days"
        )
        command[idx] = parts[5]
...
format = (
    f"|  %3s %{max_pid_length}s %{username_length}s %8s %5s %5s %9s  %-"
    + str(command_length)
    + "."
    + str(command_length)
    + "s  |"
...
line = format % ("GPU", "PID", "USER", "GPU MEM", "%CPU", "%MEM", "TIME", "COMMAND")
...
for i in range(len(pid)):
    print(
        format
        % (gpu_num[i], pid[i], user[i], gpu_mem[i], cpu[i], mem[i], time[i], command[i])
    )
